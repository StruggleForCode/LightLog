...
with open('./data/bgl_semantic_vec.json') as f:
    ...
    # 对原始语义向量进行拟合与转换，返回每个向量在30个主成分方向上的投影，保留了数据中大部分的方差信息
...
def TCN(train_x, train_y, valid_x, valid_y):
    ...
    # 同时在验证集上监控模型性能
    model.fit(train_x, train_y, batch_size=64, epochs=250, verbose=2, validation_data=(valid_x, valid_y))

    # 训练完成后，将模型保存到指定文件中
    model.save('./model/E-TCN-PCA-DIM30-EPOCH250.h5')
